...
//
//  有必要重载。因为Course类中含有指针成员变量stuList，它指向动态分配的内存。
//  如果不重载复制构造函数，使用默认的浅拷贝会导致两个Course对象指向同一块内存，
//  当一个对象被销毁时，另一个对象的stuList就会变成悬空指针，造成程序崩溃。
//
//  必须重载复制构造函数的情况：
//  （1）类中含有指针成员，且指向动态分配的内存
//  （2）类需要管理资源（如文件句柄、网络连接等）
//  （3）类需要维护引用计数或其他共享状态
//  （4）禁止对象复制时（将复制构造函数设为private）
//
//  2、在本题中，Student类的2个数据成员均设为公有成员变量，从数据安全的角度并不推荐这么设计，
//       如果把它们设为私有成员变量，会影响Course类中哪几个成员函数的正常使用，你有什么解决办法？（4分）
//
//  如果将Student的Id和Name设为私有成员变量，会影响以下Course类成员函数：
//  （1）DropStudentbyId函数：需要访问stuList[i].Id来比较学号
//  （2）PrintStudents函数：需要访问stuList[i].Id和stuList[i].Name来输出学生信息
//  （3）复制构造函数：需要访问course.stuList[i]的成员进行复制
//
//  解决办法：
//  （1）在Student类中提供公有的getter方法（如getId()、getName()）来获取私有成员的值
//  （2）将Course类声明为Student类的友元类（friend class Course），使Course可以访问Student的私有成员
//  （3）推荐做法：使用方法（1），符合封装原则，通过接口访问数据而不是直接暴露数据成员